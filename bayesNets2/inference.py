...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # Look at the inferenceByEnumeration function 
        # in inference.py for an example on how to use the desired functions. 
        # (Reminder: 
            # Inference by enumeration first joins over all the variables 
        #     and then eliminates all the hidden variables.

        #     In contrast, variable elimination interleaves join and 
        #     eliminate by iterating over all the hidden variables and 
        #     perform a join and eliminate on a single hidden variable before moving on to the next hidden variable.)
        
        #taken from inferenceByEnumeration......
        
        # grab all factors where we know the evidence variables (to reduce the size of the tables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
       # print str(currentFactorsList) + "currentFactorsList" + "\n"

        # join all factors by elimination order
        for joinVariable in eliminationOrder:
            # (factors not joined, resulting factor from joinFactors)
            tupleresult = joinFactorsByVariable(currentFactorsList, joinVariable)
            currentFactorsList = tupleresult[0]
            joinedFactor = tupleresult[1]
            #then we need to eliminate????
            # def eliminate(factor, eliminationVariable):
            if (len(joinedFactor.unconditionedVariables()) == 1):
                continue
            else:
                resultofelim = eliminate(joinedFactor, joinVariable)
                currentFactorsList.append(resultofelim)

            # currentFactorsList.append(joinedFactor)


#             Exception raised: Factor has only one unconditioned variable, so you can't eliminate 
# that variable.
 # Student sum of probability: 0.00013363831555
# ***     Correct sum of probability: 1.0
#so..normalize? ? ? ? ?  
        # currentFactorsList should contain the connected components of the graph now as factors, must join the connected components
        fullJoint = joinFactors(currentFactorsList)
        fullJoint = normalize(fullJoint)
