...
def constructBayesNet(gameState):
    ...
    #given? so for every house position and observation you get walls and get observation vars using method given
    #do we just add in the variables then to obsVars
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos

            obsVars.append(obsVar)
            #in the bayes net we have edges from foodhouse and ghosthouse to all the obs 1..n
            edges.append((FOOD_HOUSE_VAR, obsVar))
            edges.append((GHOST_HOUSE_VAR, obsVar))

    #edges from x and y to both foodhouse and ghosthouse
...
# Y positions determines how the houses are vertically oriented. 
# It models the vertical positions of both houses simultaneously, and 
# has one of four values: both-top, both-bottom, left-top, and left-bottom. "left-top" is as the name suggests:
#      the house on the left side of the board is on top, and the house on the right side of the board is on the bottom.
# Food house and ghost house specify the actual positions of the two houses. 
# They are both deterministic functions of "X positions" and "Y positions"
# The observations are measurements that Pacman makes while traveling around the board.
#  Note that there are many of these nodes---one for every board position that might be the wall of a house. 
#If there is no house in a given location, the corresponding observation is none; 
#otherwise it is either red or blue, with the precise distribution of colors depending on the kind of house. 
...
def fillObsCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    # Every possible observation position is 
    # adjacent to a possible center for a house. 
    # Pacman might observe that position to contain 
    # a red wall, a blue wall, or no wall. 
    # These outcomes occur with the following probabilities 
    # (again defined in terms of constants at the top of the file):

    # If the adjacent house center is 
    # occupied by neither the ghost house or the food house, 
    # an observation is none with certainty (probability 1).
    ...
    # IMPORTANT NOTE: the structure of the Bayes Net means that the food house 
    # and ghost house might be assigned to the same position. 
    # This will never occur in practice. 
    # But the observation CPT needs to be a proper distribution 
    # for every possible set of parents. 
    # In this case, you should use the food house distribution.
    # Hints

    # There are only four entries in the Y position factor, 
    # so you can specify each of those by hand. 
    # You'll have to be cleverer for the observation variables. 
    # You'll find it easiest to first loop over possible house positions, 
    # then over possible walls for each house, 
    # and finally over assignments to (wall color, 
    #     ghost house position, food house position) triples. 
    # Remember to create a separate factor for every one of the 
    # 4*7=28 possible observation positions.

    gamewidth = gameState.data.layout.width
    gameheight = gameState.data.layout.height

    for housePos in gameState.getPossibleHouses():

        if housePos == bottomLeftPos:
            obsPosRes = BOTTOM_LEFT_VAL
        elif housePos == bottomRightPos:
            obsPosRes = BOTTOM_RIGHT_VAL
        elif housePos == topRightPos:
            obsPosRes = TOP_RIGHT_VAL
        elif housePos == topLeftPos:
            obsPosRes = TOP_LEFT_VAL

        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsFactor = bn.Factor([obsVar], HOUSE_VARS, bayesNet.variableDomainsDict())
            
            for a in obsFactor.getAllPossibleAssignmentDicts():
                red = 0
                blue = 0
                none = 0
                #print str(a) + "a" + "\n"
                if a[FOOD_HOUSE_VAR] == obsPosRes:
                   # print str(obsPosRes) + "obsPosRes" + '\n'
                    red = PROB_FOOD_RED
                    blue = 1 - PROB_FOOD_RED
                    none = 0
                elif a[GHOST_HOUSE_VAR] == obsPosRes:
                    red = PROB_GHOST_RED
                    ...
                    blue = 0
                    none = 1
                if a[obsVar] == BLUE_OBS_VAL:
                    obsFactor.setProbability(a, blue)
                elif a[obsVar] == RED_OBS_VAL:
                    obsFactor.setProbability(a, red)
                elif a[obsVar] == NO_OBS_VAL:
                    obsFactor.setProbability(a, none)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    # Inside bayesAgents.py, 
    # use the inferenceByVariableElimination function you just wrote
    #  to compute the marginal distribution over positions of the food house, 
    #  then return the most likely position. 
    #  This information is used by Bayesian Pacman, 
    #  who wanders around randomly collecting information for a fixed number of timesteps, 
    #  then heads directly to the house most likely to contain food.

    #so.... this means the largest prob? 
    queryVariables = FOOD_HOUSE_VAR
    probfoodhousecondeverythingelse = inference.inferenceByVariableElimination(bayesNet, queryVariables, evidence, eliminationOrder)

    maxprob = 0
    #probabilities cant be more than 1 annd less than 0 right??? LOL idk
    bestPos = None
    for ass in probfoodhousecondeverythingelse.getAllPossibleAssignmentDicts():
        prob = probfoodhousecondeverythingelse.getProbability(ass)
        Pos = ass
        if prob > maxprob:
            maxprob = prob
            bestPos = Pos
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        print "elimination order " + str(eliminationOrder) + "\n"
        #util.raiseNotDefined()
        jointdist = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        for ass in jointdist.getAllPossibleAssignmentDicts():
            #idk? comment from piazza .. makes sense i guess???? 
            #since we're getting rewards associated w entering houses so either its like you win or you get a ghost

            # p(foodHouse = topLeft and ghostHouse = topRight | evidence) and 
            # p(foodHouse = topRight and ghostHouse = topLeft | evidence). 
            # Then use these two probabilities to compute expected values for rushing left and rushing right.

            if (ass['foodHouse'] == 'topLeft' and ass['ghostHouse'] == 'topRight'):
                leftExpectedValue += jointdist.getProbability(ass)*WON_GAME_REWARD
                rightExpectedValue += jointdist.getProbability(ass)*GHOST_COLLISION_REWARD
            if (ass['foodHouse'] == 'topRight' and ass['ghostHouse'] == 'topLeft'):
                leftExpectedValue += jointdist.getProbability(ass)*GHOST_COLLISION_REWARD
                rightExpectedValue += jointdist.getProbability(ass)*WON_GAME_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # util.raiseNotDefined()
        # Note that E[action | evidence] is exactly the quantity computed by computeEnterVals, 
        # so to compute the value of exploration, you can call computeEnterValues again with 
        # the hypothetical evidence provided by getExplorationProbsAndOutcomes.

        pairs = self.getExplorationProbsAndOutcomes(evidence)
        for (prob, explorationEvidence) in pairs:
            #need to get computeEnterVals with the evidence ooooo ok
            maxVal = 0
            Eactioncondevidence = self.computeEnterValues(explorationEvidence, enterEliminationOrder)
            for val in Eactioncondevidence:
                if (val > maxVal):
                    maxVal = val
