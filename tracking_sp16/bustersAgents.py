...
class GreedyBustersAgent(BustersAgent):
	...
	def chooseAction(self, gameState):
		...
		#make list of the closest ghost positions
		ghostPositions = []
		for gDist in livingGhostPositionDistributions:
			maxi = 0
			pos = (0,0)
			for key in gDist.keys():
				temp = gDist[key]
				if temp > maxi:
					maxi = temp
					pos = key
		...
		for pos in ghostPositions:
			temp = self.distancer.getDistance(pos, gameState.getPacmanPosition())
			if temp < lowestDist:
				lowestDist = temp
				closestPos = pos

		#select best action, minimizes distance to closestPos
		# legal = [a for a in gameState.getLegalPacmanActions()]
		distDict = dict()
		for action in legal:
			successorPosition = Actions.getSuccessor(pacmanPosition, action)
			kee = self.distancer.getDistance(successorPosition, closestPos)
			distDict[kee] = action
		key = min(list(distDict.keys()))
		return distDict[key]
